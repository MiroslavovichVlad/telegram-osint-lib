...
abstract class BaseAuthorization implements Authorization
{
    ...
    private NotEncryptedSocketMessenger $socketContainer;
    /**
     * @var string
     */
     ...
     */
    abstract protected function getPqInnerDataMessage($pq, $p, $q, $oldClientNonce, $serverNonce, $newClientNonce, bool $isTemp = false): TLClientMessage;
     ...
     * @throws TGException
     */
    public function createAuthKey(callable $onAuthKeyReady, bool $forRegister = false): void
    {
            ...
            $this->requestDHParams($primes, $pqResponse, function ($dhResponse) use ($onAuthKeyReady, $pqResponse, $permAuthParams) {
                ...
                $this->setClientDHParams(
                    ...
                    function (AuthParams $authKeyParams) use ($onAuthKeyReady, $permAuthParams) {
                        $nonce = openssl_random_pseudo_bytes(self::LONG_BYTES, $strong);
                        if ($strong === false || $nonce === false) {
                            throw new TGException(TGException::ERR_CRYPTO_INVALID);
                        }
                        $sessionId = openssl_random_pseudo_bytes(self::LONG_BYTES, $strong);
                        if ($strong === false || $sessionId === false) {
                            throw new TGException(TGException::ERR_CRYPTO_INVALID);
                        }
    ...
    private function encryptMessage(string $binaryMessage, int $messageId, AuthParams $authParams): string
    {
        ...
        $padding = openssl_random_pseudo_bytes($this->calcRemainder(-$length, 16));

        $payload = $data.$padding;

        [$aes_key, $aes_iv] = self::oldAesCalculate($msgKey, $authParams->getAuthKey());
     ...
     * @throws TGException
     */
    private function requestDHParams(PQ $pq, ResPQ $pqData, callable $cb, bool $isTemp = false): void
    {
        // prepare object
